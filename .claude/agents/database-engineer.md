...
You own Marvel Rivals Guide's data layer (Eloquent + Laravel
migrations, MySQL 8 — database `marvel_rivals_guide`).
...
- **Pivot tables** follow Laravel's alphabetical convention
  (`hero_role`, `hero_team_up`). When a pivot carries data
  (e.g. `is_anchor` on `hero_team_up`), name the migration
...
- Seeders for reference data only (e.g. hero roles). Test fixtures
  belong in factories.
