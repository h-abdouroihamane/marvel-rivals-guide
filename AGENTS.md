...
**Marvel Rivals Guide** is a community guide for Marvel Rivals. It
catalogs heroes, their abilities and gameplay roles, and the team-up
synergies they share across the roster. Public visitors browse and
filter; a single admin signs in to add or edit heroes.

Key facts that drive every decision below:

- **Domain / audience**: Marvel Rivals players. Tech-literate gamers;
  expect strong opinions on terminology and accuracy.
...
routes/                 # one file per resource group
  ...
  heroes.php  abilities.php  settings.php
  teamUps.php  vocabulary.php  web.php
tests/
  Feature/
  Unit/
```
...
Marvel Rivals players. Assume familiarity with the game's vocabulary
(hero, ability, ultimate, team-up, Vanguard, Duelist, Strategist).
Spell out internal-only acronyms on first use; do not over-explain
in-game terms.
...
- Confirm destructive actions (deleting a hero, removing a team-up)
...
- Never convey information by color alone (pair with icon, text, or
  pattern). Critical for role indicators on hero cards.
...
1. **Aesthetic direction** — name one: editorial, brutalist,
   ...
   This project leans into the hero-card / comic-panel aesthetic of
...
2. Copy `.env.example` to `.env`, run `php artisan key:generate`,
   ...
   `DB_DATABASE=marvel_rivals_guide`, plus `DB_USERNAME` /
   `DB_PASSWORD`), then `php artisan migrate`.
...
- Meaningful, intention-revealing names. No `tmp`, `data2`, `arr`,
  `obj`. Use the domain word (`hero`, `team-up`, `ability`).
